...
#----4. Extract Data on sentinel Grid----
#Extract MTVI
...
#----6. Test of Fractional Vegetation Cover (FVC) against vegetation indices-----
...
#----7.Vegetation Indices Comparison----
...
y <- as.vector(ARE_1_MRE_MTVI_Grid$MTVI)
df <- data.frame(x = x, y = y,
                 d = densCols(x, y, colramp = colorRampPalette(rev(c('yellow','orange','turquoise4','dodgerblue4')))))#colorRampPalette(rev(rainbow(10, end = 4/6)))))
...
#-----Plot SEQ MTVI vs Sentinel MTVI
x <- as.vector((SEN_MTVI_Grid$MTVI))
y <- as.vector(ARE_1_SEQ_MTVI_Grid$MTVI)
df <- data.frame(x = x, y = y,
                 d = densCols(x, y, colramp = colorRampPalette(rev(c('yellow','orange','turquoise4','dodgerblue4')))))#colorRampPalette(rev(rainbow(10, end = 4/6)))))
...
#-----Plot SEQ SAVI vs Sentinel SAVI
...
#-----Plot MRE SAVI vs Sentinel SAVI
x <- as.vector((SEN_SAVI_Grid$savi))
y <- as.vector(ARE_1_MRE_SAVI_Grid$savi)
df <- data.frame(x = x, y = y,
                 d = densCols(x, y, colramp = colorRampPalette(rev(c('yellow','orange','turquoise4','dodgerblue4')))))#colorRampPalette(rev(rainbow(10, end = 4/6)))))
...
PlotVIAll <-grid.arrange(SEQSENSAVI,MRESENSAVI, SEQSENMSAVI2, MRESENMSAVI2,SEQSENMTVI,MRESENMTVI, nrow = 3)#Plots of  MRE Surveys Mean Data 

ggsave(
  PlotVIAll,
  filename = "E:/glenn/Tramway Experiment/Processed/Plots/VI_SEN.png",
  width = 16,
  height = 25,
  units = "cm"
)
