...
p1sg <- ggplot(df) +
  ...
  xlab('Tramway Green band')+
  ylab('Sequoia Green Band')+
  #coord_equal(ratio=1)
...
p1sr <- ggplot(df) +
  ...
  #theme(aspect.ratio=1)+
  xlab('Tramway Red band')+
  ylab('Sequoia Red Band')+
  #coord_equal(ratio=1)
  coord_fixed(xlim=c(0,0.5),ylim=c(0,0.5))
#plot(p)
...
p1sre <- ggplot(df) +
  ...
  #ggtitle("Comparison of Tramway Mean Data with Sequoia Survey \n TRM1 RedEdge Band")+
  #theme(aspect.ratio=1)+
  xlab('Tramway RedEdge band')+
  ylab('Sequoia RedEdge Band')+
  #coord_equal(ratio=1)
  coord_fixed(xlim=c(0,0.5),ylim=c(0,0.5))
...
p1sni <- ggplot(df) +
  ...
  #theme(aspect.ratio=1)+
  xlab('Tramway NIR band')+
  ylab('Sequoia NIR Band')+
  #coord_equal(ratio=1)
  coord_fixed(xlim=c(0,0.5),ylim=c(0,0.5))
#plot(p)
...
PlotMREMEAN <-grid.arrange(pMMB, pMMG, pMMR, pMMRE,pMMN,pMMVI,pMSG, pMSR, pMSRE,pMSN,pMSVI, nrow = 2)#Plots of  MRE Surveys Mean Data 

ggsave(
  PlotMREMEAN,
  filename = "E:/glenn/Tramway_Rcode/figures/plots/All_mean_plots_one panel.pdf",
  width = 25,
  height = 16,
  units = "cm"
)
...
patchwork <-(pMMG + pMSG) /(pMMR+pMSR)/(pMMRE+pMSRE)/(pMMN+pMSN)
patchwork
ggsave(
  patchwork,
  filename = "E:/glenn/Tramway_Rcode/figures/plots/All_mean_plots_one panel.pdf",
...
ggsave(
  patchwork,
  filename = "E:/glenn/Tramway_Rcode/figures/plots/All_mean_plots_one panel.png",
  width = 16,
  height = 25,
  units = "cm"
)
...
#----27.  SEQ vs NRE Mean NDVI Plot for Figure 5----
# Plot SEQ vs MRE  NDVI
{
  x <- as.vector(TRM_Mean_SEQ_FootprintSpectralonNDVI$TRM_1_SEQ_3lines_SPE_index_ndvi)
  y <- as.vector(TRM_Mean_MRE_FootprintSpectralonNDVI$TRM_1_MRE_2lines_SPE_index_ndvi)
